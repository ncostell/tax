...
/// new URL for housing costs data
var housingCostUrl = "https://services.arcgis.com/P3ePLMYs2RVChkJx/ArcGIS/rest/services/ACS_10_14_Housing_Costs_Boundaries/FeatureServer/0";

// feature layer for the housing costs data
const housingCostLayer = new FeatureLayer({
    url: housingCostUrl,
    title: "Housing Costs",
});

// load additional housing cost data
var loadHousingCostData = function() {
    let housingCostQuery = housingCostLayer.createQuery();
    housingCostQuery.returnGeometry = false;
    housingCostQuery.outFields = ["NAME", "MEDIAN_HOUSING_COST"];

    housingCostLayer.queryFeatures(housingCostQuery).then(function(response) {
        console.log(response);
        response.features.forEach(function(feature) {
            if (feature.attributes['STUSPS'] !== "DC") { 
                ...
                if (stateObj[stateName]) {
                    stateObj[stateName].medianHousingCost = medianHousingCost;
                }
            }
        });
  loadStates()
